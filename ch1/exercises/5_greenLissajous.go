...
var palette = []color.Color{
	color.Black, 								// 黑色
	color.RGBA{G: uint8(255), A: uint8(255)},	// 绿色
}

// 常量
const (
	blackIndex = 0 // 画板中的第 0 种颜色
	greenIndex = 1 // 画板中的第 1 种颜色
)
...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {			// 外层循环 64 个迭代, 每个迭代产生一个动画帧
		...
		img := image.NewPaletted(rect, palette)			// 此时底色为黑色
		for t := 0.0; t < cycles*2*math.Pi; t += res {	// 内层循环通过设置一些像素为黑色产生一个新的图像
			x := math.Sin(t)							// x 轴是正弦函数
			y := math.Sin(t*freq + phase)				// y 轴也是正弦化的,
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), greenIndex)	// 每个内层循环通过设置一些色素为绿色产生一个新的图像
		}
		phase += 0.1
